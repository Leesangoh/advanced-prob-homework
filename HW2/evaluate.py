...
normal_pdf = lambda x, mu, sigma: 1 / (sigma * np.sqrt(2 * np.pi)) * np.exp(- (x - mu)**2 / (2 * sigma**2))
binomial_pmf = lambda k, n, p: np.math.factorial(n) / (np.math.factorial(k) * np.math.factorial(n - k)) * (p**k) * ((1 - p)**(n - k))

# By letting l = n * p, let us show that binomial pmf can be approximted by Normal pdf
...
for p in p_list:
    ...
    for i, n in enumerate(n_list):
        ...
        for k in range(0, n + 1):
            ...
            normal_pdf_list = [normal_pdf(k, mu, sigma) for k in range(0, n + 1)]

        # calculate the kullback-leibler divergence
        kl_divergence = 0
        for k in range(0, n + 1):
            if normal_pdf_list[k] == 0:
                kl_divergence += 0
            else:
                kl_divergence += binomial_pmf_list[k] * (np.log(binomial_pmf_list[k]) - np.log(normal_pdf_list[k]))
        ...
        plt.plot(range(0, n + 1), normal_pdf_list, label='Normal pdf', marker='o', color = color_per_n_list[i], markersize=5)
    
    # set legend from its color. 'red': p=0.1, 'blue': p=0.2, ..., 'brown': p=0.9
    # set text size to 20
    plt.legend(loc='upper right', labels=['n = ' + str(n) + ', ' + distr_type  for n in n_list for distr_type in ['Binomial', 'Normal']], prop={'size': 15})
    plt.title(f'Binomial pmf and Normal pdf with p = {p}', fontsize=30)
    ...
    # tighten
    plt.tight_layout()
    plt.savefig(f'binomial_pmf_normal_pdf_p_{p}.pdf', dpi = 300)
...
print("\\end{tabular}")
print("\\caption{Kullback-Leibler divergence between binomial pmf and Normal pdf}")
